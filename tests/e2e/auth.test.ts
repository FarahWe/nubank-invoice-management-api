...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/forgot-password', () => {
    beforeEach(() => {
      vitest.spyOn(sesClient, 'send').mockImplementation(() => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      const sendResetPasswordCodeEmailSpy = vitest.spyOn(
        emailService,
      ...
      expect(sendResetPasswordCodeEmailSpy).toHaveBeenCalledWith(
        accountOne.email,
        dbAccount?.password_code
      )

      expect(dbAccount?.password_code).toBeTruthy()
      expect(dbAccount?.password_reset_tries).toEqual(
        config.password.resetPasswordMaxRetries
      )
      expect(dbAccount?.reset_password_sent_at).toBeTruthy()
    })
  ...
  describe('POST /v1/auth/reset-password', () => {
    ...
    test('should return 204 and reset the password', async () => {
      ...
      const isPasswordMatch = await compare('password2', dbUser!.password)
      expect(isPasswordMatch).toBe(true)
    })

    test('should return 401 to wrong code', async () => {
      const accounts = await insertAccounts([generateAccount()])
    ...
    test('should return 401 reaching max password retires', async () => {
      const accounts = await insertAccounts([generateAccount()])
      ...
      for (
        let i = 0;
        i <
        Array.from({ length: config.password.resetPasswordMaxRetries }).length;
        i++
      ) {
        await request(app)
          .post('/v1/auth/reset-password')
      ...
      const dbAccount2 = await prisma.account.findFirst({
        where: {
          id: accountOne.id
        }
      })
    ...
    test('should return 400 if code is missing', async () => {
      await request(app)
        .post('/v1/auth/reset-password')
        .send({ password: 'password2', email: 'any@email.com' })
        .expect(BAD_REQUEST)
    })
    ...
    test('should return 400 if password is missing or invalid', async () => {
      await request(app).post('/v1/auth/reset-password').expect(BAD_REQUEST)
